...
## 2026-03-02 — Ad-hoc Codesigning over Apple Developer Account
**Context:** The built `ELVES.app` (downloaded via Chrome) showed "damaged and can't be opened" because Tauri's build produces a Mach-O binary with a linker-level ad-hoc signature, but the `.app` bundle's resources (Info.plist, icon.icns) are not sealed in that signature. macOS Gatekeeper sees an inconsistent signature and rejects the app. Chrome's quarantine flag (`com.apple.quarantine`) triggers the inspection.
**Options:** (1) Apple Developer account ($99/yr) with notarization, (2) ad-hoc codesign (`codesign --force --deep --sign -`) in CI, (3) instruct users to `xattr -cr` only
**Decision:** Ad-hoc codesign the full `.app` bundle in CI after `npx tauri build`, replacing `tauri-apps/tauri-action@v0`. Create the DMG manually with `hdiutil`. Provide Homebrew `--no-quarantine` and `xattr -cr` fallback instructions.
**Rationale:** An Apple Developer account costs $99/yr, requires annual renewal, and is overkill for an open-source project with no revenue. Ad-hoc signing seals all resources into a consistent signature that satisfies Gatekeeper's integrity check — the "damaged" error goes away. Users still see "unidentified developer" on first launch but can bypass with right-click → Open or `xattr -cr`. The `tauri-apps/tauri-action@v0` action was replaced because it creates and uploads a DMG in one step with no hook point for codesigning between build and packaging. Manual steps give us full control over the signing and DMG creation pipeline. Future: if the project grows and needs seamless first-launch, we can add a Developer account and notarization step without changing the pipeline structure.
...
## 2026-03-05 — Eager Workspace Insertion to Fix Terminal View Race Condition
**Context:** When a task starts, `useTeamSession` creates a worktree via `createWorkspace` (Tauri IPC), then calls `openWorkspace(slug)` to activate the terminal tab. But `ProjectWorkspace` resolves the active workspace by looking up `workspaces.find(w => w.slug === activeWorkspaceSlug)`. The workspace only appears in the array after the async `listWorkspaces` call resolves, which races with the synchronous `openWorkspace` call. Result: terminal view doesn't render because the lookup returns null.
**Options:** (1) Await `listWorkspaces` before `openWorkspace` — adds visible latency. (2) Add the workspace eagerly to the store before `openWorkspace`. (3) Have `ProjectWorkspace` fall back to a loading state for unknown slugs.
**Decision:** Added `addWorkspace(workspace)` to the workspace store. Both `analyzeAndDeploy` and `deployWithPlan` call it with a provisional `WorkspaceInfo` (slug, path, branch, status=active, filesChanged=0) immediately after `createWorkspace` returns, before calling `openWorkspace`. The background `listWorkspaces` refresh still runs and overwrites with full metadata.
**Rationale:** Option 2 is the simplest fix with zero latency impact. The provisional entry has enough data for the terminal view to render (slug for lookup, path for display, status for the LIVE badge). The `addWorkspace` method is idempotent — if the slug already exists, it's a no-op. The background refresh replaces the provisional entry with full metadata (accurate branch name, file counts). Option 1 would delay tab activation by 50-200ms for no user benefit. Option 3 adds UI complexity for a transient state that should never be visible.

## 2026-03-05 — Worktree Cleanup on PTY Exit via Remove Button
**Context:** When a PTY session ends (elf finishes or user stops), the worktree stays on disk indefinitely. Users accumulate orphaned worktrees under `.claude/worktrees/` over time.
**Options:** (1) Auto-remove worktree on PTY exit — dangerous, user may want to review changes. (2) Add a "Remove" button in the terminal footer on exit — explicit user action. (3) Background garbage collection on app startup — too aggressive.
**Decision:** When `hasExited` is true, the terminal footer shows a "Remove" button next to "Ship It". Clicking it calls `removeWorkspace(projectPath, slug, true)` to delete the worktree, then `wsStore.removeWorkspace(slug)` to update the store and close the tab.
**Rationale:** Explicit user action is safest. The user sees the terminal output, decides whether to "Ship It" (merge changes) or "Remove" (discard everything). Auto-cleanup would lose work if the user stepped away during a session. The force flag (`true`) handles cases where the worktree has uncommitted changes — the user chose to remove, so we respect that.
